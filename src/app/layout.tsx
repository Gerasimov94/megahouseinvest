import type { Metadata } from "next";
import "./globals.css";

export const metadata: Metadata = {
  title: "МегаХаусИнвест - Строительная компания",
  description: "Профессиональное строительство домов и коммерческих объектов. Многолетний опыт, качественные материалы, современные технологии.",
  keywords: "строительство домов, строительная компания, коттеджи, коммерческое строительство, mega house invest, мега хаус инвест",
  openGraph: {
    title: "МегаХаусИнвест - Строительная компания",
    description: "Профессиональное строительство домов и коммерческих объектов",
    type: "website",
    locale: "ru_RU",
  },
  verification: {
    google: 'qQ9UQJvK8uULQxs8jTicz4SbeNFRQkJCmvEkFphnpFs',
    yandex: 'cabf0f0daf2c9e30',
  }
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang="ru">
      <body className="antialiased">
        {children}
      </body>
    </html>
  );
}
